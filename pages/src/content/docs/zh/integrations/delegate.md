...
委托模式专为 AI 编码代理设计 — 如 Claude Code、Codex、Cursor、Open Code、Qoder 等。如果宿主 Agent 已经具备模型访问能力，使用委托模式可以直接复用宿主 Agent 的既有认证与额度进行代码审查，无需额外配置 OCR 凭据。

适用于以下场景：

1. 你的 AI 编码代理已经具备模型访问能力，希望复用已有额度进行代码审查 — 无需额外配置 OCR 凭据。
