...
委托模式专为订阅制 AI 编码代理设计 — 如 Claude Code、Codex、Cursor、Open Code、Qoder 等。这些工具已内置 LLM 订阅额度，使用委托模式可以直接复用宿主 Agent 的订阅额度进行代码审查，无需额外配置 OCR 凭据。

适用于以下场景：

1. 你的 AI 编码代理使用订阅制，希望复用已有额度进行代码审查 — 无需额外配置 OCR 凭据。
