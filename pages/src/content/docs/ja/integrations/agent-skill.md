...
SDK は SKILL.md prompt をロードし、agent が[skill が行うこと](#what-the-skill-does)で述べられているワークフローを実行します——`npm install` のフォールバックや、選択したローカル runner が無い、またはログアウトしているときに認証を促す手順を含みます。
