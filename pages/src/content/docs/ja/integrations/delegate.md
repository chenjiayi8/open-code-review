...
デリゲーションモードは、AI コーディングエージェント向けに設計されています — Claude Code、Codex、Cursor、Open Code、Qoder など。これらのツールでホストエージェントがすでにモデルへアクセスできる場合、デリゲーションモードではその既存認証と利用枠をコードレビューに活用できます。追加のモデル設定や API キーは不要です。

以下の場合に使用してください：

1. AI コーディングエージェントがすでにモデルへアクセスでき、その利用枠をコードレビューに再利用したい場合 — 追加の API キーやモデル設定は不要。
