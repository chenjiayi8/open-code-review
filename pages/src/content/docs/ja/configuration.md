...
OCR はプロバイダーのエンドポイント、モデル、API キーを保存しません。レビュー実行はサブスクリプション対応で、OCR はインストール済みのローカル runner を呼び出し、その runner が認証を管理します。
...
ローカルのサブスクリプションログインは CI に自動では移りません。CI ワークフローでは OCR をインストールし、ジョブ内で選択した Codex または Claude runner を認証します。OCR にプロバイダーエンドポイントや API キー設定コマンドを追加しないでください。
