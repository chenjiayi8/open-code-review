...
1. **Проверка предварительных требований.** Выполнить `which ocr`, чтобы
   убедиться, что CLI находится в `PATH`, затем `ocr review --preview`, чтобы проверить
   доступность LLM.
...
3. **Остановка и запрос при ненастроенной LLM.** Если `ocr review --preview`
   ...
   runner login) и ждёт аутентификации выбранного runner.
