...
> Однако локальный runner необходимо аутентифицировать заранее: навык не может войти в Codex или Claude за вас и
...
1. **Проверка предварительных требований.** Выполнить `which ocr`, чтобы
   убедиться, что CLI находится в `PATH`, затем `ocr review --preview`, чтобы проверить цель ревью до запуска runner.
...
3. **Остановка и запрос при неаутентифицированном runner.** Если preflight runner или ревью завершается ошибкой из-за отсутствующего Codex/Claude или отсутствия аутентификации, агент *не* придумывает учётные данные. Он просит пользователя аутентифицировать выбранный установленный runner поддерживаемым нативным способом (существующий вход или API-токен).
...
`npm install` и запрос аутентификации, если выбранный локальный runner отсутствует или не вошёл в аккаунт.
