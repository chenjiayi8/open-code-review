...
Режим делегирования предназначен для AI-агентов программирования,
таких как Claude Code, Codex, Cursor, Open Code, Qoder и другие, где доступ
к LLM уже есть у основного агента. Вместо настройки отдельного эндпоинта
модели для OCR вы используете существующую аутентификацию и квоту основного агента для
проведения ревью.

Используйте режим делегирования, когда:

1. Ваш AI-агент программирования уже имеет доступ к модели, и вы хотите использовать
   эту квоту для код-ревью без дополнительной настройки OCR credentials.
...
Аутентификация runner на стороне OCR не требуется: режим делегирования
никогда не вызывает Codex или Claude через OCR.
...
|------|-------------------|----------|
| [Навык агента](../agent-skill/) | OCR | Агент вызывает `ocr review --runner codex`; OCR управляет полным процессом ревью. |
