...
OCR не управляет учётными данными runner или настройками подключения провайдера. Сначала аутентифицируйте установленную CLI runner поддерживаемым нативным способом (существующий вход или API-токен):
...
Локальное состояние аутентификации (существующий вход или API-токен) принадлежит Codex или Claude CLI на данной машине. CI jobs должны аутентифицировать выбранный runner внутри CI; не добавляйте настройку учетных данных провайдера OCR.
