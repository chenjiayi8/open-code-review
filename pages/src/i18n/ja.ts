...
export const ja: TranslationKeys = {
  ...
  'features.feat3Desc': 'ログイン済みのローカル Codex または Claude サブスクリプション CLI でレビューを実行します。OCR はファイル選択、ルール適用、並行処理、コメント位置決めを担当し、認証は runner が管理します。',
  'features.feat4Title': '動的並行処理',
  'features.feat4Desc': 'Runner 認証可能な goroutine ワーカー（デフォルト8）でサブタスクを動的に分割し並列レビュー。大規模な変更セットでも迅速に完了します。',
  ...
  'quickstart.subtitle': 'OCR 側の認証情報設定は不要です。インストール済みの Codex または Claude CLI の既存 native login または対応 API token を使用します。',
  ...
  'quickstart.step2Title': 'Runner 認証',
  'quickstart.step2Desc': 'Codex または Claude にログインし、OCR コマンドごとに runner を選択します。',
  'quickstart.step2Label1': '対話式セットアップ（推奨）',
  'quickstart.step2Label2': '手動Runner 認証（代替）',
  'quickstart.step2Label3': 'Runner 認証確認',
  ...
  'docs.configTitle': 'Runner 認証と検証',
  'docs.configDesc': 'インストール済みローカル runner を native login または対応 API token で認証します。OCR はエンドポイント、モデル、認証情報を保存しません。',
  'docs.configInteractive': '対話式セットアップ（推奨）',
  'docs.configInteractiveDesc': 'OCR 実行前に codex login で Codex runner を認証します。',
  'docs.configModelSelect': 'モデル選択',
  'docs.configModelSelectDesc': 'Claude runner で OCR を実行する前に、Claude CLI の native login または対応 API token で認証します。',
  'docs.configListProviders': '必須 Runner フラグ',
  'docs.configListProvidersDesc': 'preview 以外の review または scan では --runner codex または --runner claude を指定します。',
  'docs.configManual': '手動Runner 認証',
  'docs.configManualBuiltin': 'Runner を選択',
  'docs.configManualBuiltinDesc': '--runner でこの実行に使う認証済みローカル CLI を選択します。',
  'docs.configManualCustom': '任意の Runner モデル',
  'docs.configManualCustomDesc': 'プライベートデプロイメントや他の互換エンドポイントに接続します。',
  'docs.configManualCustomNote': '--preview などの読み取り専用/プリフライトコマンドは runner を呼び出しません。',
  'docs.configKeys': 'サポートされるRunner 認証キー',
  'docs.configKeyProvider': 'Runner 名: codex または claude',
  'docs.configKeyProviderApiKey': '認証は runner CLI に保存されます',
  'docs.configKeyProviderModel': '単発実行用の任意 runner モデル',
  'docs.configKeyProviderUrl': 'OCR 側のエンドポイント上書きなし',
  'docs.configKeyProviderExtraBody': 'OCR 側の資格情報ボディフィールドなし',
  'docs.configKeyProviderExtraHeaders': 'OCR 側の資格情報ヘッダーなし',
  'docs.configKeyLanguage': '出力言語',
  'docs.configKeyTelemetry': 'テレメトリ',
  'docs.configVerify': 'Runner 認証確認',
  'docs.configVerifyDesc': 'Runner 認証後にこのコマンドを実行して LLM 接続をテストし、Runner 認証が正しいことを確認します。',
  ...
  'docs.reviewFlag7Desc': 'local runner プロセス全体のタイムアウト（分）',
  'docs.reviewFlag8Desc': '単一タスクタイムアウト（分）',
  'docs.reviewFlag9Desc': '出力対象：human（プロセス表示）または agent（サマリーのみ）',
  'docs.reviewFlag10Desc': '選択した local runner に渡す任意のモデルヒント',
  ...
  'docs.scanFlag10Desc': '選択した local runner に渡す任意のモデルヒント',
  'docs.scanFlag11Desc': 'local runner プロセス全体のタイムアウト（分）',
  'docs.scanFlag12Desc': '出力対象：human（進捗表示）または agent（サマリーのみ）',
  'docs.scanFlag13Desc': 'スキャンに関する任意の要件・業務背景情報',
  'docs.scanFlag14Desc': 'local runner の任意のプロセスタイムアウト',
  'docs.scanFlag14Default': '内蔵',
  'docs.scanFlag15Desc': '最大並行 git サブプロセス数',
  'docs.scanFlag16Desc': 'カスタム JSON レビュールールのパス',
  'docs.scanFlag17Desc': '選択した local runner が読み取り専用権限を管理',
  ...
  'docs.mcpConfig': 'MCP Server のRunner 認証',
  'docs.mcpConfigLocation': 'MCP server のRunner 認証は ~/.opencodereview/config.json に保存されます。',
  'docs.mcpDelete': 'MCP Server の削除',
  'docs.mcpFields': 'Runner 認証フィールド',
  ...
  'docs.envDesc': 'すでに Claude Code ユーザーで以下の環境変数をRunner 認証済みの場合、Open Code Review は自動的に認識します。追加Runner 認証は不要です：',
  'docs.envNote': '<code>ocr config</code> を使用してこれらのRunner 認証を上書きまたは補完することもできます。',
  'docs.copy': 'コピー',
  ...
  'docs.sidebar.configuration': 'Runner 認証',
