...
OCR запускается через установленную CLI Codex или Claude. Аутентифицируйте выбранную CLI поддерживаемым нативным способом (существующий вход или API-токен); сам OCR не хранит учётные данные runner.
...
Локальная настройка описана в [руководстве по конфигурации](https://open-codereview.ai/docs/configuration). Аутентификация в CI выполняется отдельно: CI-задача должна аутентифицировать выбранный runner нативным способом (существующий вход или API-токен), а не хранить учётные данные OCR.
...
# OCR отвечает за выбор файлов и разрешение правил; настройка runner для OCR не требуется
...
- Режимы выполнения ревью — после интеграции выберите способ выполнения ревью
  - [Локальный runner](https://open-codereview.ai/docs/configuration) — OCR выполняет ревью через нативно аутентифицированную CLI Codex или Claude
